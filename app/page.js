...
function CleanupTab() {
  ...
  const fmtNum = v => v == null ? 'â€”' : '$' + Math.round(v).toLocaleString('en-US');
    ...
    <div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="rounded-xl p-4" style={{background:'white', border:'1px solid #f5c6c6'}}>
          ...
          <div className="text-xs mt-1" style={{color:'#8899aa'}}>{fmtNum(totals.liab.total)} total Â· {allRows.filter(r => r.type === 'liability').length} items</div>
        </div>
        <div className="rounded-xl p-4" style={{background:'white', border:'1px solid #9dd4b5'}}>
          ...
          <div className="text-xs mt-1" style={{color:'#8899aa'}}>{fmtNum(totals.asset.total)} total Â· {allRows.filter(r => r.type === 'asset').length} items</div>
        </div>
        <div className="rounded-xl p-4" style={{background: NAVY, border:`2px solid ${GOLD_ACCENT}`}}>
          ...
          <div className="text-xs mt-1" style={{color:'rgba(255,255,255,0.5)'}}>Liabilities âˆ’ Assets {totals.netNeeded < 0 ? '(surplus)' : ''}</div>
        </div>
              ...
              return (
                <tr key={r.id} style={{borderBottom:'1px solid #f0f4f8', textDecoration: r.resolved ? 'line-through' : 'none', opacity: r.resolved ? 0.6 : 1, background: isAsset ? '#f9fdf9' : 'white'}}>
                  ...
                  <td className="px-3 py-2 text-xs" style={{color:'#6b7a99', maxWidth: 400}}>
                    {isEditing ? <textarea value={editForm.notes} onChange={e => setEditForm({...editForm, notes: e.target.value})} rows={2} className="w-full text-xs rounded border px-1 py-0.5" style={{borderColor:'#dde4ed', color: NAVY}} /> : (r.notes ? <span title={r.notes}>{r.notes.length > 80 ? r.notes.slice(0, 80) + 'â€¦' : r.notes}</span> : 'â€”')}
                  </td>
                  <td className="px-2 py-2 text-center">
                    {r.isDebt ? (
                      ...
                      <div className="flex gap-1 justify-center">
                        ...
                        <button onClick={() => { setEditingId(null); setEditForm(null); }} className="text-xs px-1.5 py-0.5 rounded" style={{background:'#6b7a99', color:'white'}}>Ã—</button>
                      </div>
                    ) : (
                      <div className="flex gap-1 justify-center">
...
function DebtScheduleTab() {
  const [debts, setDebts] = useState(() => {
    ...
    try {
      ...
      // Migrate legacy records â€” drop removed IDs, fill missing fields
      const migrated = parsed
        ...
        .map(d => {
          ...
          // Migrate legacy docFile (string) â†’ docFiles (array)
          ...
          // like the legacy BS-derived figure (~$2.5M). Idempotent â€” once balance
          ...
          // (10% APR placeholder â€” pending LOC agreement). Idempotent: only fills
          ...
          // computed EFA-style balance (48 remaining Ã— $3,144.03) + addendum-
          ...
          // use M Ã— n; traditional bank loans show principal).
  ...
  const fmtRate = r => r == null ? 'â€”' : (r * 100).toFixed(2) + '%';
  const fmtDate = d => d ? new Date(d).toLocaleDateString('en-US', { month: 'numeric', day: 'numeric', year: 'numeric' }) : 'â€”';
  const fmtNum = v => v == null ? 'â€”' : '$' + Math.round(v).toLocaleString('en-US');
    ...
    <div>
      ...
      <div className="rounded-xl p-3 mb-4 flex items-center gap-3" style={{background:'white', border:'1px solid #dde4ed'}}>
        ...
        <span className="text-xs ml-auto" style={{color:'#8899aa'}}>
          {filtered.length} of {debts.length} <span style={{color:'#cbd5e0'}}>Â·</span> {debts.filter(d => d.active).length} active, {debts.filter(d => !d.active).length} inactive
        </span>
      </div>
            ...
            {filtered.map(d => {
              ...
              return (
                <tr key={d.id} onClick={() => openDetails(d)} className="cursor-pointer hover:bg-gray-50" style={{borderBottom:'1px solid #f0f4f8', textDecoration: inactive ? 'line-through' : 'none', opacity: inactive ? 0.7 : 1}}>
                  <td className="px-2 py-2 text-center">
                    <input type="checkbox" checked={d.active} onClick={e => e.stopPropagation()} onChange={e => setDebts(prev => prev.map(x => x.id === d.id ? {...x, active: e.target.checked} : x))} title={d.active ? 'Active â€” uncheck to mark inactive' : 'Inactive â€” check to mark active'} />
                  </td>
                  ...
                  <td className="px-2 py-2 text-center" style={{color: txtColor}}>{d.termMonths || 'â€”'}</td>
                  ...
                  <td className="px-3 py-2 text-right" style={{color: txtColor}}>{d.payoff != null ? fmtNum(d.payoff) : 'â€”'}</td>
                  <td className="px-2 py-2 text-right" style={{color: inactive ? '#cbd5e0' : '#8899aa'}}>{d.active && totals.grand > 0 && d.balance ? ((d.balance / totals.grand) * 100).toFixed(1) + '%' : 'â€”'}</td>
                  <td className="px-2 py-2 text-center">{d.docFiles?.length ? <span style={{color: GOLD_ACCENT}}>ðŸ“„{d.docFiles.length > 1 ? ' Ã—' + d.docFiles.length : ''}</span> : <span style={{color:'#cbd5e0'}}>â€”</span>}</td>
                </tr>
              );
            })}
            <tr style={{background: NAVY, color:'white', borderTop:'2px solid '+GOLD_ACCENT}}>
              <td className="px-3 py-2 font-bold" colSpan={5}>Total {filterEntity !== 'All' ? '(' + filterEntity + ')' : 'Long-Term Debt'} {statusFilter !== 'All' ? 'â€” ' + statusFilter : ''}{filterType !== 'All' ? ' â€” ' + filterType : ''}</td>
              ...
              <td className="px-3 py-2 text-right font-bold" style={{color: GOLD_ACCENT}}>{(() => { const s = filtered.reduce((acc, d) => acc + (d.payoff || 0), 0); return s > 0 ? fmtNum(s) : 'â€”'; })()}</td>
              <td className="px-2 py-2 text-right font-bold" style={{color: GOLD_ACCENT}}>{statusFilter === 'Active' && filterType === 'All' && filterEntity === 'All' ? '100.0%' : 'â€”'}</td>
              <td></td>
            </tr>
          </tbody>
        </table>
          ...
          <div onClick={e => e.stopPropagation()} className="rounded-xl overflow-hidden flex flex-col" style={{background:'white', border:'1px solid #dde4ed', width:'95vw', maxWidth:1400, height:'90vh'}}>
            <div className="px-5 py-3 flex items-center justify-between" style={{background: NAVY, color:'white', borderBottom:'2px solid '+GOLD_ACCENT}}>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setSelected(null)} className="px-2 py-1 rounded text-sm" style={{background:'rgba(255,255,255,0.2)', color:'white'}}>âœ•</button>
              </div>
            </div>
              ...
              <div className="overflow-y-auto p-5 border-r" style={{borderColor:'#dde4ed'}}>
                {!editing ? (
                  <div className="space-y-3 text-xs">
                    ...
                    <DetailRow label="Term (Months)" value={selected.termMonths || 'â€”'} />
                    ...
                    <DetailRow label="Early Payoff" value={selected.payoff != null ? fmtNum(selected.payoff) : 'â€”'} />
                    <DetailRow label="Documents" value={selected.docFiles?.length ? selected.docFiles.length + ' file' + (selected.docFiles.length > 1 ? 's' : '') : 'â€” (drop PDFs in /public/loan-docs/ and reference by filename)'} />
                    {selected.notes && (
  ...
  if (!files || files.length === 0) {
    return (
      <div className="flex-1 flex items-center justify-center p-8 text-center">
        <div>
          <div className="text-4xl mb-3" style={{color:'#cbd5e0'}}>ðŸ“„</div>
